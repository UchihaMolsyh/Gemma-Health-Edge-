...
Every time someone Googles "chest pain" or types symptoms into ChatGPT, they are sending their most sensitive personal data to a corporation's servers. People in rural areas, low-income households, and privacy-sensitive situations deserve an intelligent health assistant that **works offline, costs nothing, and keeps their data to themselves.**

**Gemma Health Edge is that assistant.**

It's a full-stack AI health platform powered by **Google's Gemma 4 AI models** running locally via llama.cpp (and ollama on pc web application) — a web dashboard you open in any browser, a native Flutter app on Android/iOS, and a hardened Python backend. Everything runs on hardware you already own.
...
- 🚨 **Life-saving safety layer** — detects 100+ emergency patterns (cardiac arrest, overdose, suicide ideation) and responds with the correct emergency number for the user's country *before* the AI speaks
...
```
 ...
 │  • 10 languages (i18n)           │   │    (on-device LLM — no server!) │
 │  • Mood calendar + chat history  │   │  • Offline CSV RAG search       │
 │  • PDF export, RLHF thumbs       │   │  • TFLite on-device ML          │
 │  • Camera + voice input          │   │  • Encrypted Hive storage       │
 │  • Real-time GPU/TPS dashboard   │   │  • Voice input + TTS            │
 └────────────────┬─────────────────┘   └──────────────┬──────────────────┘
                  │                                     │
 ...
 │                                                                          │
 │  1. 🚨 Emergency Detector  ──→ if triggered: return crisis numbers NOW  │
 │       (100+ regex patterns: cardiac, overdose, suicide, stroke, etc.)   │
 │                                                                          │
 │  2. 👋 Greeting Filter     ──→ if "hello/hi/hola/etc": fast reply, done│
 │       (40+ greetings across 15 languages, skips full LLM call)          │
 │                                                                          │
 │  3. 🔬 Research Engine     ──→ Wikipedia + PubMed fetched in parallel   │
 │       (MD5-cached 2 hrs · only fires on medical keyword queries)        │
 │                                                                          │
 │  4. 🩺 Clinical Profile    ──→ "Patient: Age 34, Diabetes, Metformin"  │
 ...
 │       (anatomy errors, miracle cures, "stop your medication" etc.)      │
 │                                                                          │
 │  7. 🧠 LLM Guardrail       ──→ Gemma 4 validates its own response      │
 │       (circuit breaker: trips after 3 failures, resets after 2 min)    │
 │                                                                          │
 │  8. 📊 RLHF Logging        ──→ thumbs up/down saved to votes.jsonl     │
 │                                                                          │
 │  SQLite DB: chat_sessions · mood_entries · clinical_profiles (WAL mode)│
 └──────────────────────────────┬───────────────────────────────────────────┘
   ...
   │  Q4_K_M GGUF    │ │  gemma-4-9b │ │  gemma-4-9b    │ │  gemma-4-27b │
   │  5.3 GB model   │ │  local only │ │  gemma-4-27b   │ │  cloud only  │
   │  32K context    │ │             │ │  cloud API     │ │              │
...
Upload a photo of a rash, pill bottle, or wound — Gemma 4's vision model describes what it sees. Or speak your symptoms out loud with voice input. Both work on the web app and the Flutter mobile app.

### Feature 3 — Personalised Clinical Profile 🩺
Enter your allergies, conditions, and medications once. Every single AI prompt silently includes this context: *"Patient: Age 34, Conditions: Type 2 Diabetes, Medications: Metformin 500mg"* — making every response tailored to you, with no extra effort.

### Feature 4 — Research-Grounded Answers 🔬
For medical questions, the system fetches real context from **Wikipedia** and **PubMed** in parallel, caches it, and injects it into the prompt — so Gemma 4's answers are grounded in actual medical literature, not just its training data.
...
git clone <repo-url>
cd  gemma-health-edge "The file location"
